 ...
 */
package ru.surfstudio.android.demo.core.mvi.mapper
 ...
 *
 * Пагинация с помощью paging3 обрабатывается отдельно
 * */
    ...
    object ErrorHandlers {
        ...
        fun <T> loadingBased(errorHandler: ErrorHandler): RequestErrorHandler<T> =
            { error, _, loading ->
                ...
                val isErrorStateOnDisplay =
                    loading?.isOneOf(LoadStateType.Error, LoadStateType.NoInternet) ?: false
                if (isNonAuthorized || !isErrorStateOnDisplay) {
                    error?.also(errorHandler::handleError)
                }
                true
            }
